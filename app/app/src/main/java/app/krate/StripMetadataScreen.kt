...
@Composable
fun StripMetadataScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Button(
            onClick = {
                val input = "${inputPath.trim()} | ${outputPath.trim()}"
                isLoading = true
                coroutineScope.launch {
                    val res = withContext(Dispatchers.IO) {
                        Core.run("StripMetadata", input)
                    }
                    output = res.text
                    isError = !res.ok
                    isLoading = false
            ...
            if (isLoading) {
                ...
                Text("Strip Metadata", style = MaterialTheme.typography.titleLarge)
            }
        }
